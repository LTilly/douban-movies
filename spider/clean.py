...
for mov in movies:
	...
	# 可以用简易清洗函数清洗的部分信息，若存在则清洗，不存在则存空字符
	if '导演' in mov:
		director=cleanitem('导演')
	else:
		director=''
	if '编剧' in mov:
		composer=cleanitem('编剧')
	else:
		composer=''
	if '主演' in mov:
		actor=cleanitem('主演')
		...
		actor=''
	if '类型' in mov:
		category=cleanitem('类型')
	else:
		category=''
	if '语言' in mov:
		language=cleanitem('语言')
	else:
		language=''
	if '又名' in mov:
		name=cleanitem('又名')
	...
	# 地区信息的清洗，每个地区同时存入中英文
	if '制片国家/地区' in mov:
		district=mov['制片国家/地区'].split('/')
		temp=''
		for item in district:
			item=item.strip()
			if item in nameMap:
				temp=temp+nameMap[item]+'_'+item+'/'
		district=temp[:-1]
	else:
		district=''
	...
	# 简介的清洗
	if 'description' in mov:
		description=cleanitem('description')
	else:
		description=''

	# 信息的存储
	cleaned.append({'id':mid,'title':title,'rate':rate,'url':url,'cover':cover,'director':director,'composer':composer,'actor':actor,'category':category,'district':district,'language':language,'name':name,'time':time,'length':length,'description':description})

# 将清洗后的信息存入json
f2=open('cleanedMovie.json','w')
